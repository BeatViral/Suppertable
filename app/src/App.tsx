...
function App() {
  ...
  const activeTitle = useMemo(() => view === 'admin' ? 'Admin' : view === 'account' ? 'Your kitchen' : 'Founding Kitchens', [view])
  ...
  return <div className="app-shell">
    ...
    <main><div className="eyebrow">SSU Australia · Serve Something Unique.</div><h1>{activeTitle}</h1>
      ...
      {loading ? <p>Loading your secure workspace…</p> : view === 'apply' ? <ApplyView profile={profile} application={application} openAuth={openAuth} done={(a) => { setApplication(a); setProfile((p) => p ? { ...p, role: 'kitchen_founder' } : p); setNotice({ kind: 'success', text: 'Your application has been received. We’ll be in touch personally.' }); navigate('account') }} /> : view === 'account' ? <AccountView profile={profile} application={application} openAuth={openAuth} /> : <AdminView applications={applications} selected={selected} setSelected={setSelected} refreshed={loadData} notify={setNotice} />}
    </main>
